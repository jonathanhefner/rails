...
class ActiveModelI18nTests < ActiveModel::TestCase
  ...
  def test_translated_model_name_with_count_when_i18n_key_missing
  ...
  def test_translated_model_name_with_count_when_i18n_key_missing_and_custom_plural_rule
    I18n.backend = Class.new(I18n::Backend::Simple) do
      include I18n::Backend::Pluralization
    end.new

    I18n.backend.store_translations "en", i18n: { plural: { rule: -> (count) { count != 1 ? :one : :other } } }

    assert_equal "Person", Person.model_name.human(count: 0)
    assert_equal "People", Person.model_name.human(count: 1)
