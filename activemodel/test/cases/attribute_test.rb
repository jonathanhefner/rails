...
module ActiveModel
  class AttributeTest < ActiveModel::TestCase
    ...
    test "from_user + value_for_database uses serialize_after_cast when explicitly defined" do
    ...
    test "from_user + value_for_database does not use serialize_after_cast when defined by superclass" do
      type = Class.new(Type::Value) do
        def serialize_after_cast(value)
          "bad"
        end
      ...
      attribute = Attribute.from_user(nil, "whatever", Class.new(type).new)

      assert_equal "whatever", attribute.value_for_database
    end

    test "from_user + value_for_database does not use serialize_after_cast when defined by delegate class" do
      type = Class.new(Type::Value) do
        def serialize_after_cast(value)
          "bad"
        end
      ...
      attribute = Attribute.from_user(nil, "whatever", DelegateClass(Type::Value).new(type.new))
