...
module ActiveModel
  class AttributeTest < ActiveModel::TestCase
    ...
    test "from_user + value_for_database uses serialize_cast_value when explicitly defined" do
      ...
      assert_equal "good(cast(whatever))", attribute.value_for_database
    end

    test "from_user + value_for_database does not use serialize_cast_value when not explicitly defined by subclass" do
      base_type = Class.new(Type::Value) do
        def serialize_cast_value(value)
          "bad"
        end
      end

      attribute = Attribute.from_user(nil, "whatever", Class.new(base_type).new)

      assert_equal "whatever", attribute.value_for_database
    end

    test "from_user + value_for_database does not use serialize_cast_value when not explicitly defined by delegate class" do
      base_type = Class.new(Type::Value) do
        def serialize_cast_value(value)
          "bad"
        end
      ...
      attribute = Attribute.from_user(nil, "whatever", DelegateClass(Type::Value).new(base_type.new))
