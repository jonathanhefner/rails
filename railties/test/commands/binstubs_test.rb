...
class Rails::Command::BinstubsTest < ActiveSupport::TestCase
  ...
  test "binstubs:change can be run from the `rails` gem command" do
  ...
  test "binstubs:change can target a specified directory" do
    create_windows_binstubs "foo/bar", "bin/foo"

    assert_no_changes -> { read_file "bin/foo" } do
      run_change_command "foo"
    end

    assert_binstub "foo/bar"
  end
  ...
  test "binstubs:change can target multiple specified files and directories" do
    create_windows_binstubs "bin/foo", "bin/bar/baz", "bin/qux"

    assert_no_changes -> { read_file "bin/qux" } do
      run_change_command "bin/foo", "bin/bar"
    end
