...
class Rails::Command::DockerTest < ActiveSupport::TestCase
  ...
  test "docker:render renders with CI support enabled by default" do
    app_file "config/Dockerfile.erb", %(<%= ci? %>)
    run_render_command
    assert_match "true", read_file("Dockerfile")
  end

  test "docker:render --skip-ci-support disables CI support" do
    app_file "config/Dockerfile.erb", %(<%= ci? %>)
    run_render_command skip_ci_support: true
    assert_match "false", read_file("Dockerfile")
  end
  ...
  private
    def run_render_command(template: nil, dockerfile: nil, environment: nil, skip_ci_support: nil, **options)
      args = []
      ...
      args.push("--skip-ci-support") if skip_ci_support
