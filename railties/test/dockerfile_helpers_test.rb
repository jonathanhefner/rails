...
class DockerfileHelpersTest < ActiveSupport::TestCase
  ...
  test "#render re-uses the current rendering context" do
    app_file "config/ci.dockerfile.erb", %(<%= ci? %>)

    helpers.ci = false
    assert_equal "false", helpers.render("config/ci.dockerfile.erb").chomp

    helpers.ci = true
    assert_equal "true", helpers.render("config/ci.dockerfile.erb").chomp
  end

  test "#render considers template path relative to the app root" do
    app_file "my/dockerfile/partial.erb", %(<%= "stuff".upcase %>)

    Dir.chdir app_path("config") do
      assert_equal "STUFF", helpers.render("my/dockerfile/partial.erb").chomp
    end
  end
