*   Credentials commands (e.g. `bin/rails credentials:edit`) now respect
    ...
    Before:
      ...
      * If `config.credentials.content_path` or `config.credentials.key_path`
        was set, `bin/rails credentials:edit` could not be used.  Editing
        credentials required using `bin/rails encrypted:edit path/to/credentials
        --key path/to/key`.
    ...
    After:
      ...
      * `bin/rails credentials:edit --environment foo` will create
        `config/credentials/foo.yml.enc` _if_ `config.credentials.content_path`
        is not set for the `foo` environment.  It will then edit the credentials
        file that the app would load for the `foo` environment.
