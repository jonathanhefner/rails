...
module Rails
  module DockerfileHelpers
    ...
    def initial_stage(name = "initial", base: "ruby:#{ruby_version}-slim")
      stage from: base, as: name do
        "ENV BUNDLE_APP_CONFIG=.bundle"
      end
    end

    def runtime_packages_stage(name = "runtime_packages", base: "initial")
      stage from: base, as: name do
        install_packages runtime_packages, skip_recommends: true
      end
    end

    def buildtime_packages_stage(name = "buildtime_packages", base: "runtime_packages")
      stage from: base, as: name do
        install_packages buildtime_packages
      end
    end

    def gems_stage(name = "gems", base: "buildtime_packages", bundle_without: ["development"])
      stage from: base, as: name do
        ["WORKDIR staged", "COPY Gemfile Gemfile.lock .", install_gems(without: bundle_without)]
      end
    end

    def node_modules_stage(name = "node_modules", base: "buildtime_packages")
      stage from: base, as: name do
        [install_node, "", "WORKDIR staged", "COPY package.json yarn.lock .", install_node_modules]
      end
    end

    def final_stage(base: "runtime_packages", user:, workdir:, gems_stage: "gems", node_modules_stage: "node_modules")
      stage from: base do
        [
          "RUN useradd #{user}",
          "USER 1000:1000", "",
          "WORKDIR #{workdir}", "",
          copy("staged", from: gems_stage, chown: "1000:1000"),
          (copy("staged", from: node_modules_stage, chown: "1000:1000") if node?),
    ...
    def install_packages(*packages, skip_recommends: false, cache: true)
      run apt_install(packages, skip_recommends: skip_recommends), mounts: (apt_cache_mounts if cache)
    end
    ...
    private
      def stage(from:, as: nil, &block)
        content = Array(block.call).compact.join("\n").presence

        [ "FROM #{from}", (" as #{as}" if as),
          ("\n\n" if content),
          content
        ].compact.join
      end
      ...
      def apt_cache_mounts
        mounts :cache, ["/var/cache/apt", "/var/lib/apt"], sharing: "locked"
      end

      def precompilation_mounts
        mounts :cache, ["tmp/cache/assets"], uid: 1000, gid: 1000
      end
