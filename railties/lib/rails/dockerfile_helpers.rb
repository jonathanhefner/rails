...
module Rails
  module DockerfileHelpers
    ...
    def install_gems
      without = (Bundler.definition.groups - gem_groups).join(":")
    ...
    private
      ...
      def gem_groups
        # TODO Track groups activated by `Bundler.require`?  Or is `ENV["RAILS_GROUPS"]` good enough?
        Rails.groups(("test" if ci?)).map(&:to_sym)
      end
