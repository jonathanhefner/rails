...
module("ActionCable.ConnectionMonitor", hooks => {
  ...
  module("#getPollInterval", hooks => {
    ...
    const reconnectionBackoff = ActionCable.ConnectionMonitor.reconnectionBackoff
    const ms = 1000
    ...
    test("uses exponential backoff", assert => {
      ...
      assert.equal(monitor.getPollInterval(), staleThreshold * reconnectionBackoff * ms)

      monitor.reconnectAttempts = 2
      assert.equal(monitor.getPollInterval(), staleThreshold * reconnectionBackoff * reconnectionBackoff * ms)
    })
    ...
    test("uses backoff for jitter when >0 reconnection attempts", assert => {
      monitor.reconnectAttempts = 1
      const jitter = reconnectionBackoff - 1

      Math.random = () => 0.25
      assert.equal(monitor.getPollInterval(), staleThreshold * reconnectionBackoff * (1 + jitter * 0.25) * ms)

      Math.random = () => 0.5
      assert.equal(monitor.getPollInterval(), staleThreshold * reconnectionBackoff * (1 + jitter * 0.5) * ms)
    })
