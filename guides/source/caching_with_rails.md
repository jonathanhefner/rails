...
Options passed to the cache constructor will be treated as default options for the relevant API methods.
...
`config.cache_store` is not explicitly set.
...
[`ActiveSupport::Cache::NullStore`][] scoped to each web request, and clears stored values at the end of a request. It is meant for use in development and test environments. It can be very useful when you have code that interacts directly with `Rails.cache` but caching interferes with seeing the results of code changes.
