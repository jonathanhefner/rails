...
module CacheStoreCompressionBehavior
  ...
  included do
    ...
    test "compression works with custom coder" do
      @cache = lookup_store(compress: true, coder: Marshal)
    ...
    test "compression by default when using custom coder and cache format version >= 7.1" do
    ...
    test "compression is disabled by default when using custom coder and cache format version < 7.1" do
      @cache = ActiveSupport::Cache.with(format_version: 7.0) do
        lookup_store(coder: Marshal)
      end
