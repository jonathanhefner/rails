...
module MessagePackSharedSerializerTests
  ...
  included do
    ...
    test "raises error when registering an already-registered type ID" do
      assert_raises(match: /registered/i) do
        serializer.register_type 0, File, packer: proc { "" }, unpacker: proc { nil }
      end
    end
