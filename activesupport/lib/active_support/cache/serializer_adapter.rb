...
module ActiveSupport
  module Cache
    class SerializerAdapter # :nodoc:
      def initialize(serializer:, compressor:, delegate_entire_dump: false)
        @serializer = serializer
        @compressor = compressor
        @delegate_entire_dump = delegate_entire_dump
        @serializer_supports_compression = @serializer.respond_to?(:dump_compressed)
      end

      def dump(entry)
        return serialize(entry) if @delegate_entire_dump

        dump_compressed(entry, Float::INFINITY)
      end

      def dump_compressed(entry, threshold)
        return serialize(entry, threshold) if @delegate_entire_dump
        ...
        if type = type_for_string(entry.value)
          ...
          payload = serialize(Cache::Entry.new(entry.value))
        end
      ...
      def load(dumped)
        return deserialize(dumped) if !signature?(dumped)
      ...
      private
        ...
        def serialize(value, compression_threshold = Float::INFINITY)
          if @serializer_supports_compression && compression_threshold != Float::INFINITY
