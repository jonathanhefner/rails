...
module ActiveSupport
  class Deprecation
    module Disallowed
      ...
      # +allowed_warnings+ can be an array containing strings, symbols, or
      # regular expressions. (Symbols are treated as strings.) These are
      # compared against the text of deprecation warning messages generated
      # within the block. Matching warnings will be exempt from the rules set by
      # #disallowed_warnings.
      #
      # The +if:+ option accepts a boolean or an object that responds to +.call+.
      # If it evaluates to true, then matching warnings will be allowed.
      # Otherwise the block will behave as if it were written without +allow+.
      #
      #   ActiveSupport::Deprecation.disallowed_behavior = :raise
      #   ActiveSupport::Deprecation.disallowed_warnings = ["something broke"]
      #
      ...
      #     ActiveSupport::Deprecation.warn("something broke!")
      #   end
      #   # => nil for production, ActiveSupport::DeprecationException for dev / test
