  ...
  module SignedId
    ...
    module ClassMethods
      ...
      def find_signed(signed_id, purpose: nil)
        ...
        check_signed_id_fresh(*signed_id_verifier.verified(signed_id, purpose: combine_signed_id_purposes(purpose))) do |id|
      ...
      def find_signed!(signed_id, purpose: nil)
        check_signed_id_fresh(*signed_id_verifier.verify(signed_id, purpose: combine_signed_id_purposes(purpose))) do |id|
          find id
      ...
      # :nodoc:
      def check_signed_id_fresh(id = nil, expires_with = nil, fresh_value = nil)
        model = yield id
        model if expires_with.nil? || model.send(expires_with) == fresh_value
      end
    ...
    def signed_id(expires_in: nil, expires_with: nil, purpose: nil)
      payload = expires_with ? [ id, expires_with, self.send(expires_with) ] : id
