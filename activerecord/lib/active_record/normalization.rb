...
module ActiveRecord # :nodoc:
  ...
  module Normalization
    ...
    module ClassMethods
      ...
      # ==== Options
      #
      # * <tt>:with</tt> - The normalization to apply. May be specified as a
      #   Proc or as one or more objects that respond to +to_proc+. If more than
      #   one object is specified, the resulting Procs will be pipelined
      ...
      def normalizes(*names, including_nil_values: false, with:)
        with = Array(with).map(&:to_proc).reduce(&:>>)
    ...
    private
      ...
      class NormalizedValueType < DelegateClass(ActiveModel::Type::Value) # :nodoc:
        ...
        def serialize(value)
          super(value.is_a?(CastResult) ? value.result : cast(value))
        end
